...
export const registerPhase51: Phase = ({ register }) => {
    register('piService', (c: IContainer) => new PiService(dalOf(c)));
    register('zedService', (c: IContainer) => new ZedService(dalOf(c), llmOf(c)));
    register('warpService', (c: IContainer) => new WarpService(dalOf(c), llmOf(c)));
    register('gptEngineerService', (c: IContainer) => new GptEngineerService(llmOf(c), c.has('toolRunnerService') ? c.get<IToolRunnerService>('toolRunnerService') : undefined));
    register('gooseService', (c: IContainer) => new GooseService(dalOf(c)));
    register('continueService', (c: IContainer) => new ContinueService(llmOf(c)));
    register('tabbyService', (c: IContainer) => new TabbyService(dalOf(c)));
    register('gptPilotService', (c: IContainer) => new GptPilotService(dalOf(c)));
    register('voidService', (c: IContainer) => new VoidService(dalOf(c), llmOf(c)));
    register('crushService', () => new CrushService());
    register('codeWhaleService', (c: IContainer) => new CodeWhaleService(c.has('toolRunnerService') ? c.get<IToolRunnerService>('toolRunnerService') : undefined));
};
