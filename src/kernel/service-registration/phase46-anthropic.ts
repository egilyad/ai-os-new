...
export const registerPhase46: Phase = ({ register }) => {
    ...
    register('filesApiService', (c:IContainer)=> new FilesApiService(c.get<DataAccessLayer>('dal')));
    register('cacheControlService', (c:IContainer)=> new CacheControlService(c.get<DataAccessLayer>('dal')));
    register('projectService', (c:IContainer)=> new ProjectService(c.get<DataAccessLayer>('dal'), llmOf(c)));
    ...
    register('routineService', (c:IContainer)=> new RoutineService(c.get<DataAccessLayer>('dal')));
    register('agentViewService', (c:IContainer)=> new AgentViewService(c.get<DataAccessLayer>('dal'), c.has('toolRunnerService')?c.get<IToolRunnerService>('toolRunnerService'):undefined));
};
