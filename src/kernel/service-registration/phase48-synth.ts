...
export const registerPhase48: Phase = ({ register }) => {
    register('sciAgentsService', (c: IContainer) => new SciAgentsService(dalOf(c)));
    register('sparksService', (c: IContainer) => new SparksService(dalOf(c), llmOf(c)));
    register('aiScientistService', (c: IContainer) => new AiScientistService(dalOf(c), llmOf(c), c.has('toolRunnerService') ? c.get<IToolRunnerService>('toolRunnerService') : undefined));
    register('latentService', (c: IContainer) => new LatentService(dalOf(c), eventsOf(c), llmOf(c)));
    register('eightStageService', (c: IContainer) => new EightStageService(dalOf(c)));
    register('cognitaeService', (c: IContainer) => new CognitaeService(dalOf(c), llmOf(c)));
    register('cogTeamService', (c: IContainer) => new CogTeamService(dalOf(c), llmOf(c)));
    register('synService', (c: IContainer) => new SynService(dalOf(c)));
    register('helixService', (c: IContainer) => new HelixService(dalOf(c)));
    register('ideatorService2', (c: IContainer) => new IdeatorService(dalOf(c), llmOf(c)));
};
