...
export const registerPhase47: Phase = ({ register }) => {
    ...
    register('analitikService', (c: IContainer) => new AnalitikService(dalOf(c), llmOf(c)));
    ...
    register('heisenbergService', (c: IContainer) => new HeisenbergService(dalOf(c)));
    register('agencyRuService', (c: IContainer) => new AgencyRuService(dalOf(c), c.has('crewService') ? c.get<ICrewService>('crewService') : undefined));
