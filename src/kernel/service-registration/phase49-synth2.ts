...
export const registerPhase49: Phase = ({ register }) => {
    register('rckService', (c: IContainer) => new RckService(dalOf(c)));
    register('cogneeService', (c: IContainer) => new CogneeService(dalOf(c), llmOf(c)));
    register('metanService', (c: IContainer) => new MetanService(dalOf(c)));
    register('conceptsService', (c: IContainer) => new ConceptsService(dalOf(c)));
    register('secondBrainService', (c: IContainer) => new SecondBrainService(dalOf(c)));
    register('stormService2', (c: IContainer) => new StormService(dalOf(c), llmOf(c), c.has('knowledgeService') ? c.get<import('../contracts/parity').IKnowledgeService>('knowledgeService') : undefined));
    register('blackboardService2', (c: IContainer) => new BlackboardService(dalOf(c)));
    register('metaControllerService2', (c: IContainer) => new MetaControllerService(eventsOf(c)));
    register('doloresService2', (c: IContainer) => new DoloresService(dalOf(c)));
    register('epistemeService2', (c: IContainer) => new EpistemeService(dalOf(c)));
};
