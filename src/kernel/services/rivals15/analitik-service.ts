...
export class AnalitikService implements IAnalitikService {
    ...
    async intake(request: string){
        ...
        try{ this.events?.emit(EVENTS.ANALITIK_INTAKE, { id }); }catch{}
        return `Proposal ${id}:\n${proposal.slice(0,1500)}`;
    }
    private async ask(prompt: string){
        if (this.llm) { try { const r=await this.llm.chat([{role:'system',content:'Ты агент Analitik Lab (GigaChat).'},{role:'user',content:prompt.slice(0,3000)}],{temperature:0.4,maxTokens:600}); if(!r.error) return r.content; } catch {} }
