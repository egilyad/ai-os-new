    ...
    for (const participant of participants) {
        ...
        for (const chain of chains) {
            for (const step of chain.steps) {
                if (step.type === 'claim') {
                    // Preserve existing evidence/citations if step carries them (mineClaims / FactCheck path) — A-01 + A-07
                    const maybe = step as unknown as { evidence?: string; citations?: string[]; source?: string };
                    claims.push({
                        ...
                        role: participant.role ?? (step as unknown as { role?: string }).role ?? '',
                        evidence: maybe.evidence ?? maybe.source,
                        citations: maybe.citations,
                    });
                }
            }
        }
...
export class DebateConsensusEngine implements IConsensusEngine {
    ...
    /** D2.2 — ArgTech bonus (set via setArgTechBonus, no new runtime). */
    private argTechBonus: number | null = null;

    /** D2.2 — minimal wiring: caller may set ArgTech-derived bonus (Dung/Toulmin/Brier) without creating second pipeline. */
    setArgTechBonus(bonus: number | null): void {
        this.argTechBonus = bonus !== null ? Math.max(-0.2, Math.min(0.2, bonus)) : null;
        if (bonus !== null) this.lastClaimsHash = null; // invalidate cache when bonus changes
    }

    evaluate(claims: Claim[]): ConsensusResult {
        const hash = claims.map((c) => `${c.id}:${c.confidence.toFixed(3)}`).join('|') + `|b:${this.argTechBonus ?? 'x'}`;
        ...
        let confidence = this.calculateConfidence(agreements, conflicts, claims);
        // D2.2 — ArgTech bonus (Dung grounded / Toulmin completeness / Brier) within existing 0..1 clamp
        if (this.argTechBonus !== null) confidence = Math.max(0, Math.min(1, confidence + this.argTechBonus));
