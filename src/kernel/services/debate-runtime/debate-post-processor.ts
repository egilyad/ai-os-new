...
export class DebatePostProcessor {
    ...
    // R-GAP-03: separate Set for FactCheck — was sharing processedArgIds with governor feeding (dead bridge path)
    ...
    clearProcessedIds(): void {
        ...
        this.processedFactCheckIds.clear();
        this.fedContents = [];
    }
    ...
    // N4c: processFactCheck now awaits checks (bounded per-arg), sampling policy unchanged
    // R-GAP-03: uses processedFactCheckIds (not processedArgIds) — governor feeding no longer suppresses FactCheck bridge
    async processFactCheck(newArgs: DebateArgument[]): Promise<void> {
        const pending: Promise<void>[] = [];
        for (const arg of newArgs) {
            if (this.processedFactCheckIds.has(arg.id)) continue;
            this.processedFactCheckIds.add(arg.id);
            if (this.processedFactCheckIds.size > MAX_PROCESSED_IDS) {
                const first = this.processedFactCheckIds.values().next().value;
                if (first !== undefined) this.processedFactCheckIds.delete(first);
            }
            // Await each check (bounded by checkArgument's internal timeout), sampling unchanged
            pending.push(
                this.factCheckService.checkArgument(arg).then(() => {}).catch((e) =>
                    LOGGER.warn('DebatePostProcessor', 'Fact-check failed', { error: e }),
                ),
            );
        }
        // Await all in parallel (bounded by slowest), but don't block beyond 5s total
        if (pending.length > 0) {
            await Promise.race([
