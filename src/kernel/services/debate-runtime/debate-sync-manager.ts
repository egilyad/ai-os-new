...
export class DebateSyncManager {
    ...
    /**
     * Previous implementation wiped argument `content` for old rounds under memory pressure
     * (totalBytes > 256KB), destroying the traceable debate artifact.
     * FIX(debate-text-truncation): keep full content; log only. If memory pressure
     * ever becomes real, paginate or compress instead of wiping.
     */
    truncateArguments(_entry: SyncEntry, _keepRounds = 2): number {
        // No-op: preserve full debate text for traceability.
        // Keep method for backwards compatibility; callers check return value (0 = no wipe).
        return 0;
    }
    ...
    async startDebate(
        ...
        const session = await this._initEngineSession(
            entry,
    ...
    // N4c-fix: async — _syncSessionImpl is async since mergeAndProcessSession awaits FactCheck;
    // reading entry.activeSession synchronously after fire-and-forget void caused "No active session after sync"
    private async _initEngineSession(
        entry: SyncEntry,
        topology: DebateTopology,
        topic: string,
        participants: DebateParticipant[],
        ...
        await this._syncSessionImpl(entry);
    ...
    destroy(): void {
        for (const entry of [...this._entries.values()]) {
            ...
            if (entry.activeSession && this.deps) {
                ...
                // FIX: keep arguments for potential restore; only clear on explicit destroy
                // Previously this wiped content before history save (now save preserves content).
            }
    ...
    private _syncSession(entry: SyncEntry, immediate = false): void {
        ...
        // _initEngineSession awaits _syncSessionImpl directly (not via immediate flag).
        if (!immediate) {
    ...
    private async _syncSessionImpl(entry: SyncEntry): Promise<void> {
        ...
        entry.syncing = true;
        try {
            const { session, newArgs } = await mergeAndProcessSession(
                this.engine,
    ...
    private _finalizeInternal(entry: SyncEntry): void {
        ...
        // Keep full argument content for persistence and traceability.
        // Previously content was wiped here before saveToDebateHistory to reduce clone cost,
        // which destroyed history. Now we persist full text, then emit.
        ...
        // Persist full content (no stripping) BEFORE emitting events
