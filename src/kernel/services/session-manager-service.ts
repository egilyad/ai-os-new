...
export class SessionManagerService implements ISessionManager {
    ...
    private static stripArgumentContent(session: DebateSession): DebateSession {
        // FIX(debate-text-truncation): preserve full argument content for history.
        // Previous implementation wiped `content` to '' to reduce clone cost, which
        // destroyed the traceable artifact and made history reopen show truncated/empty text.
        // IndexedDB can handle ~50KB per debate (5 debates * ~10KB) without OOM.
        return session;
    }
