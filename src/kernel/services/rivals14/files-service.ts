...
export class FilesApiService implements IFilesApiService {
    ...
    async upload(name: string, content: string){ const id=genId('file'); await this.dal.kv.set(`files/${id}`, { id, name: name.slice(0,120), content: content.slice(0,200000) }); return id; }
