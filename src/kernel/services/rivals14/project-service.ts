...
export class ProjectService implements IProjectService {
    ...
    async createProject(name: string, instructions=''){ const id=genId('proj'); await this.dal.kv.set(`projects/${id}`, { id, name: name.slice(0,120), instructions: instructions.slice(0,2000), files: [] as string[] }); try{ this.events?.emit(EVENTS.PROJECT_CREATED, { projectId: id, name: name.slice(0,120) }); }catch{} return id; }
