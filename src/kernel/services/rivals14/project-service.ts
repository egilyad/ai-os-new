...
export class ProjectService implements IProjectService {
    ...
    async createProject(name: string, instructions=''){ const id=genId('proj'); await this.dal.kv.set(`projects/${id}`, { id, name: name.slice(0,120), instructions: instructions.slice(0,2000), files: [] as string[] }); try{ this.events?.emit(EVENTS.PROJECT_CREATED_RIVALS, { projectId: id, name: name.slice(0,120) }); }catch{} return id; }
    async addFile(projectId: string, name: string, content: string){ const p=await this.dal.kv.get<Record<string,unknown>>(`projects/${projectId}`); if(!p) throw new Error('project not found'); const files=(p as Record<string,unknown>).files as string[]; files.push(`${name}:${content.slice(0,500)}`); await this.dal.kv.set(`projects/${projectId}`, p); try{ this.events?.emit(EVENTS.PROJECT_FILE_ADDED, { projectId, name: name.slice(0,120) }); }catch{} }
