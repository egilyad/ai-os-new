...
export class FactCheckService {
    ...
    // N4c: track in-flight checks so callers can await already-assigned work (bounded, sampling unchanged)
    private pendingChecks = new Map<string, Promise<ArgumentFactCheck | null>>();
    ...
    async checkArgument(
        ...
        const pending = this.pendingChecks.get(arg.id);
        ...
        const checkPromise = (async (): Promise<ArgumentFactCheck | null> => {
        ...
        this.pendingChecks.set(arg.id, checkPromise);
        checkPromise.finally(() => { this.pendingChecks.delete(arg.id); });

        return checkPromise;
    }

    /**
     * N4c: await in-flight fact-checks for given argument IDs (bounded, sampling unchanged).
     * Returns after timeout even if some checks are still pending.
     */
    async awaitPending(argumentIds: string[], timeoutMs = 2000): Promise<void> {
        const pending = argumentIds
            .map((id) => this.pendingChecks.get(id))
            .filter((p): p is Promise<ArgumentFactCheck | null> => !!p);
