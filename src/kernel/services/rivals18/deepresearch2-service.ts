...
export class DeepResearch2Service implements IDeepResearch2Service {
    ...
    async run(topic: string){
        ...
        for(const s of stages){
            if (this.llm) {
                try { const r=await this.llm.chat([{role:'system',content:`You are ${s} for deep research.`},{role:'user',content:context.slice(0,1500)}],{temperature:0.4,maxTokens:400}); if(!r.error) context=r.content; } catch {}
            }
        ...
        try{ this.events?.emit(EVENTS.DEEPRESEARCH2_RUN, { topic: topic.slice(0,200) }); }catch{}
