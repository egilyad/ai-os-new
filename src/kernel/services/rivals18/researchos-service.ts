...
export class ResearchOsService implements IResearchOsService {
    ...
    async init(){ LOGGER.info('ResearchOS', 'init',{}); } async destroy(){}
    async createFolder(name: string){ const id=genId('ros'); await this.dal.kv.set(`research-os/${id}`, { id, name: name.slice(0,80), files: [] as string[] }); try{ this.events?.emit(EVENTS.RESEARCHOS_FOLDER, { folder: name.slice(0,80) }); }catch{ /* best-effort */ } return id; }
