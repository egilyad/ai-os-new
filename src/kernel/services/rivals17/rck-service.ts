...
export class RckService implements IRckService {
    ...
    async bind(a: string, b: string){ const v=bindVec(hv(a),hv(b)); await this.dal.kv.set(`rck/bind/${a}~${b}`, v.slice(0,8)); try{ this.events?.emit(EVENTS.RCK_BIND, { id: `${a}~${b}` }); }catch{ /* best-effort */ } return `bind(${a}·${b})`; }
