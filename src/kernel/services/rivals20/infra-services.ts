...
export class HeliconeService implements IHeliconeService {
    ...
    async log(request: string){ await this.dal.kv.set(`helicone/${Date.now()}`, request.slice(0,500)); try{ this.events?.emit(EVENTS.HELICONE_LOG, { request: request.slice(0,200) }); }catch{ /* best-effort */ } }
...
export class PortkeyService implements IPortkeyService {
    ...
    async route(model: string){ const cfg=await this.dal.kv.get<string>(`portkey/${model}`) ?? 'openai'; try{ this.events?.emit(EVENTS.PORTKEY_ROUTE, { model }); }catch{ /* best-effort */ } return cfg; }
...
export class LiteLlmService implements ILiteLlmService {
    ...
    async init(){} async destroy(){}
    async proxy(model: string, prompt: string){ const res = `[LiteLLM ${model}] ${prompt.slice(0,100)}`; try{ this.events?.emit(EVENTS.LITELLM_PROXY, { model }); }catch{ /* best-effort */ } return res; }
}
export class LangfuseService implements ILangfuseService {
    ...
    async init(){} async destroy(){}
    async trace(name: string, data: string){ await this.dal.kv.set(`langfuse/${name}/${Date.now()}`, data.slice(0,1000)); try{ this.events?.emit(EVENTS.LANGFUSE_TRACE, { name }); }catch{ /* best-effort */ } }
