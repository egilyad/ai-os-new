...
export class AgentManagementService implements IAgentManagementService {
    ...
    async create(input: CreateAgentInput): Promise<AgentRecord> {
        ...
        await this.db.agentManaged.put(agent as unknown as Record<string, unknown>);
    ...
    async getBySlug(slug: string): Promise<AgentRecord | undefined> {
        ...
        return (all as unknown as AgentRecord[]).find(a => a.slug === slug);
    }

    async list(filters?: AgentFilters): Promise<AgentRecord[]> {
        let all = await this.db.agentManaged.toArray() as unknown as AgentRecord[];
        if (filters) {
    ...
    async update(id: string, input: UpdateAgentInput): Promise<AgentRecord> {
        const existing = await this.db.agentManaged.get(id) as unknown as AgentRecord | undefined;
        if (!existing) throw new Error(`Agent ${id} not found`);

        const patch: Record<string, unknown> = { ...input, updatedAt: Date.now() };
        await this.db.agentManaged.update(id, patch);
        const updated = (await this.db.agentManaged.get(id)) as unknown as AgentRecord;
    ...
    async setError(id: string, error: string): Promise<AgentRecord> {
        ...
        return (await this.db.agentManaged.get(id)) as unknown as AgentRecord;
    }
    ...
    async setParent(childId: string, parentId: string | null): Promise<AgentRecord> {
        const child = await this.db.agentManaged.get(childId) as unknown as AgentRecord | undefined;
        if (!child) throw new Error(`Agent ${childId} not found`);
        if (parentId) {
            const parent = await this.db.agentManaged.get(parentId) as unknown as AgentRecord | undefined;
        ...
        return (await this.db.agentManaged.get(childId)) as unknown as AgentRecord;
    }

    async getChildren(parentId: string): Promise<AgentRecord[]> {
        const all = await this.db.agentManaged.toArray() as unknown as AgentRecord[];
    ...
    async addSkill(agentId: string, skillId: string, config?: Record<string, unknown>): Promise<AgentSkillRecord> {
        ...
        await this.db.agentSkills.put(record as unknown as Record<string, unknown>);
        return record;
    }

    async removeSkill(agentId: string, skillId: string): Promise<void> {
        const all = await this.db.agentSkills.toArray() as unknown as AgentSkillRecord[];
    ...
    async updateSkillConfig(agentId: string, skillId: string, config: Record<string, unknown>): Promise<AgentSkillRecord> {
        ...
        return (await this.db.agentSkills.get(match.id)) as unknown as AgentSkillRecord;
    }
    ...
    async addTool(agentId: string, toolId: string, approvalMode?: string): Promise<AgentToolRecord> {
        const record: AgentToolRecord = {
            ...
            agentId,
            toolId,
            approvalMode: (approvalMode || 'auto') as ToolApprovalMode,
            enabled: true,
            addedAt: Date.now(),
        };
        await this.db.agentTools.put(record as unknown as Record<string, unknown>);
        return record;
    }

    async removeTool(agentId: string, toolId: string): Promise<void> {
        const all = await this.db.agentTools.toArray() as unknown as AgentToolRecord[];
    ...
    async setResponsibility(agentId: string, title: string, description: string, kpis?: string[]): Promise<ResponsibilityRecord> {
        ...
        await this.db.agentResponsibilities.put(record as unknown as Record<string, unknown>);
        return record;
    }

    async removeResponsibility(agentId: string, respId: string): Promise<void> {
        const all = await this.db.agentResponsibilities.toArray() as unknown as ResponsibilityRecord[];
    ...
    async recordMetric(agentId: string, metricType: MetricType, value: number, periodStart?: number, periodEnd?: number): Promise<AgentMetricRecord> {
        ...
        await this.db.agentMetrics.put(record as unknown as Record<string, unknown>);
        return record;
    }

    async getMetrics(agentId: string, metricType?: MetricType, from?: number, to?: number): Promise<AgentMetricRecord[]> {
        const all = await this.db.agentMetrics.toArray() as unknown as AgentMetricRecord[];
        return all.filter(m => {
    ...
    async addMemory(agentId: string, type: MemoryType, content: string, metadata?: Record<string, unknown>): Promise<AgentMemoryRecord> {
        ...
        await this.db.agentMemory.put(record as unknown as Record<string, unknown>);
        return record;
    }

    async getMemories(agentId: string, type?: MemoryType): Promise<AgentMemoryRecord[]> {
        const all = await this.db.agentMemory.toArray() as unknown as AgentMemoryRecord[];
        return all.filter(m => {
            if (m.agentId !== agentId) return false;
    ...
    async startExecution(agentId: string, trigger: TriggerType, taskSummary: string, provider?: string, model?: string): Promise<AgentExecutionRecord> {
        ...
        await this.db.agentExecutions.put(record as unknown as Record<string, unknown>);
    ...
    async completeExecution(executionId: string, status: ExecutionStatus, result?: string, error?: string, costUsd?: number): Promise<AgentExecutionRecord> {
        const existing = await this.db.agentExecutions.get(executionId) as unknown as AgentExecutionRecord | undefined;
        ...
        const updated = (await this.db.agentExecutions.get(executionId)) as unknown as AgentExecutionRecord;
    ...
    async listExecutions(agentId: string, status?: ExecutionStatus, limit = 50): Promise<AgentExecutionRecord[]> {
        const all = await this.db.agentExecutions.toArray() as unknown as AgentExecutionRecord[];
    ...
    async saveRevision(agentId: string, patch: Partial<UpdateAgentInput>, note?: string): Promise<AgentConfigRevisionRecord> {
        const agent = await this.db.agentManaged.get(agentId) as unknown as AgentRecord | undefined;
        ...
        await this.db.agentConfigRevisions.put(record as unknown as Record<string, unknown>);
    ...
    async listRevisions(agentId: string): Promise<AgentConfigRevisionRecord[]> {
        const all = await this.db.agentConfigRevisions.toArray() as unknown as AgentConfigRevisionRecord[];
    ...
    async rollbackRevision(agentId: string, revisionId: string): Promise<AgentRecord> {
        ...
        const agent = (await this.db.agentManaged.get(agentId)) as unknown as AgentRecord;
        ...
        return (await this.db.agentManaged.get(agentId)) as unknown as AgentRecord;
    }
    ...
    async createApiKey(agentId: string, provider: string, keyHash: string, expiresAt?: number): Promise<AgentApiKeyRecord> {
        ...
        await this.db.agentApiKeys.put(record as unknown as Record<string, unknown>);
        return record;
    }
    ...
    async listApiKeys(agentId: string): Promise<AgentApiKeyRecord[]> {
        const all = await this.db.agentApiKeys.toArray() as unknown as AgentApiKeyRecord[];
    ...
    async setBudget(agentId: string, limitUsd: number, period: string, alertThreshold = 0.8): Promise<AgentBudgetRecord> {
        ...
        if (existing) {
            ...
            return (await this.db.agentBudgets.get(existing.id)) as unknown as AgentBudgetRecord;
        }
        ...
        await this.db.agentBudgets.put(record as unknown as Record<string, unknown>);
        return record;
    }

    async getBudget(agentId: string): Promise<AgentBudgetRecord | undefined> {
        const all = await this.db.agentBudgets.toArray() as unknown as AgentBudgetRecord[];
    ...
    async listBudgetIncidents(agentId: string): Promise<BudgetIncidentRecord[]> {
        ...
        if (!budget) return [];
        const all = await this.db.budgetIncidents.toArray() as unknown as BudgetIncidentRecord[];
    ...
    async exportAgent(id: string): Promise<ExportedAgent> {
        const agent = (await this.db.agentManaged.get(id)) as unknown as AgentRecord | undefined;
        ...
        return {
            ...
            memory: await this.getMemories(id),
            exportedAt: Date.now(),
            version: '1',
        };
    }
