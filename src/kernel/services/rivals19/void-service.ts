...
export class VoidService implements IVoidService {
    ...
    async session(file: string){ const id=genId('void'); await this.dal.kv.set(`void/${id}`, { file: file.slice(0,200), history: [] as string[] }); try{ this.events?.emit(EVENTS.VOID_SESSION, { file: file.slice(0,200) }); }catch{ /* best-effort */ } return id; }
    async assist(sessionId: string, prompt: string){
        ...
        if (this.llm) { try { const r=await this.llm.chat([{role:'system',content:'You are Void editor assistant.'},{role:'user',content:prompt.slice(0,1000)}],{temperature:0.4,maxTokens:500}); if(!r.error) out=r.content; } catch { /* best-effort */ } }
