...
export class ZedService implements IZedService {
    ...
    async openBuffer(path: string, content=''){ const id=genId('zedbuf'); await this.dal.kv.set(`zed/${id}`, { path: path.slice(0,200), content: content.slice(0,5000) }); try{ this.events?.emit(EVENTS.ZED_OPEN, { path: path.slice(0,200) }); }catch{} return id; }
