            ...
            {isExpanded && (
                ...
                >
                    ...
                    {(session.arguments ?? []).slice(-5).map((arg) => {
                        const isArgExpanded = expandedArgs.has(arg.id);
                        const needsTruncate = arg.content.length > 200;
                        const display = isArgExpanded ? arg.content : arg.content.slice(0, 200);
                        return (
                            <div
                                key={arg.id}
                                style={{
                                    ...
                                    color: 'var(--text-main)',
                                    lineHeight: 1.4,
                                    cursor: needsTruncate ? 'pointer' : undefined,
                                    whiteSpace: 'pre-wrap',
                                    wordBreak: 'break-word',
                                }}
                                onClick={() => needsTruncate && toggleArg(arg.id)}
                                title={needsTruncate ? (isArgExpanded ? 'Свернуть' : 'Развернуть полный текст') : undefined}
                            >
                                <span style={{ fontWeight: 600, color: '#a855f7' }}>
                                    {arg.agentName}
                                </span>
                                : {display}
                                {needsTruncate && !isArgExpanded ? '...' : ''}
                                {needsTruncate && (
                                    <span
                                        style={{
                                            marginLeft: 6,
                                    ...
                                    >
                                        {isArgExpanded ? ' — свернуть' : ' — развернуть'}
                                    </span>
                                )}
                            </div>
                        );
